...
<script>
		...
		<div class="row">	
			<div class="col-3" align="left">   
				...
				<div class="input-group  input-group-sm date" id="text1">
					...
					<input type="text" name="text1" size="10" value="{{ $text1 }}" class="form-control" placeholder="찾을 이름?"
    ...
    <tr>
		...
		<td align="left">  <a href="{{ route('jangbuo.show', $row->id) }}{{ $tmp }}"> {{ $row->product_name29 }} </a> </td>
		<td align="right"> {{ number_format($row->price29) }}</td>
		<td align="right">{{ number_format($row->numo29) }}</td>
